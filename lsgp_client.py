...
def _send_request(url, values=None, method=None):
    """
    ...
    #Create opener using our above auth_handler
    opener = urllib2.build_opener(auth_handler)

    #Encode data and create request
...
def _wait_for_job(jobid):
    ...
    duration = 10
    while True:
        #Retrieve state for all jobs, and convert to 
...
def _save_job_result(jobid):
    ...
    @param jobid:
    """
    #Retrieve the produced gzipped tarfile    
    content = _send_request(URL_JOBS + jobid)
    outputfile = '{0}.tgz'.format(jobid)
